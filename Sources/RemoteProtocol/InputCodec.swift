...
extension ByteBuffer {
  ...
  mutating func readInputCommand() throws -> Command {
    ...
    case 1:
      guard let value = NavigationCommand(wireValue: try read(UInt8.self)) else {
        throw RemoteProtocolError.malformedMessage
      }
      return .navigation(value)
    case 2:
      guard let value = ActionCommand(wireValue: try read(UInt8.self)) else {
        throw RemoteProtocolError.malformedMessage
      }
      return .action(value)
    case 3: return .editing(try readTextEvent())
    case 4: return .application(CommandID(try readStringValue()))
  ...
  mutating func readTextEvent() throws -> TextEditEvent {
    switch try read(UInt8.self) {
    case 0: return .insert(try readStringValue())
